...
function NotificationDropdownMA() {
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [showAllNotification, setShowAllNotification] = useState(false);
  const { prasentAccounts, getAllAccountsUsersData } =
  ...
  const getNotificationsData = async () => {
    //console.log("prasentAccounts", prasentAccounts);
    try {
      const API_URL = `${process.env.NEXT_PUBLIC_EMBOT_API}/notify/get-notifications-ma`;
      const res = await fetch(API_URL, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(prasentAccounts),
      });
  ...
  const updateNotificationStatus = async (id) => {
    // console.log("id update call", maJWTToken);
    try {
      let res = await fetch(
        `${process.env.NEXT_PUBLIC_EMBOT_API}/notify/update-notification/${id}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${maJWTToken}`,
            "Content-Type": "application/json",
          },
        }
      );
  ...
  useEffect(() => {
    if (socketMA.current) {
      socketMA.current.on("newNotification", (notifyData) => {
      ...
      socketMA.current.on("stopNotificationRing", () => {
        stop();
        setDropdownOpen(false);
      });
    }
    return () => socketMA.current.off();
  }, [notificationsData, socketMA]);
...
export default NotificationDropdownMA;
