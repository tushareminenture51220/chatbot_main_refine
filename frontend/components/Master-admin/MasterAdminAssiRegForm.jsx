...
const MasterAdminAssiRegForm = ({ setShowForm }) => {
  const { userIdMA } = useMasterAdminAuth();
  const { getMasterAdminAssistants } = useMasterAdminCommonData();
  ...
  const handleSubmit = (e) => {
    ...
    formData?.masterAdminId &&
      formDataToSend.append("masterAdminId", formData.masterAdminId);
    ...
    const API_PATH = `${process.env.NEXT_PUBLIC_EMBOT_API}/master/create-assistant`;
    fetch(API_PATH, {
      method: "POST",
      body: formDataToSend,
    })
  ...
  useEffect(() => {
    if (userIdMA) {
      setFormData({ ...formData, masterAdminId: userIdMA });
    }
  }, [userIdMA]);
  return (
    <div
...
export default MasterAdminAssiRegForm;
