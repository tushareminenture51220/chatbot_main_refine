...
import MasterAdminAssiRegForm from "@/components/Master-admin/MasterAdminAssiRegForm";
  ...
  const deleteAssistants = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_EMBOT_API}/master/deleteMultipleAssistants`,
        {
          method: "DELETE",
          headers: {
      ...
      if (response.ok) {
        ...
        getMasterAdminAssistants(userIdMA);
        console.log("result", result);
        toast.success(result.message);
      } else {
        toast.error("No assistants were deleted.");
      }
    } catch (error) {
    ...
    <div className="relative overflow-x-hidden overflow-y-auto h-[80vh] shadow-md sm:rounded-lg w-full">
      <div className="flex items-center justify-between gap-4 pb-4 bg-white px-2 pt-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-semibold"> Master Admin Assistance List</h1>
        </div>
      ...
      {assistantsMA.length == 0 && (
        <div className="text-center w-full bg-gray-100 text-gray-700 p-4 rounded shadow-md">
      ...
      {showForm && <MasterAdminAssiRegForm setShowForm={setShowForm} />}
    </div>
  );
};

export default MasterAdminAssistance;
